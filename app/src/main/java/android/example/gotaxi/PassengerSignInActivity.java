...
public class PassengerSignInActivity extends AppCompatActivity {
    ...
    private Button loginSignIpButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loginSignIpButton = findViewById(R.id.loginSignIpButton);
    ...
    private boolean validateEmail(){
        ...
        if (emailInput.isEmpty()){
            textInputEmail.setError("Проверьте свой email");
            return false;
        }else {
    ...
    private boolean validateName(){
        ...
        if (nameInput.isEmpty()){
            textInputName.setError("Проверьте имя");
            return false;
        } else if (nameInput.length()>15){
            textInputName.setError("Не более 15 символов");
            return false;
        } else {
    ...
    private boolean validatePassword() {
        ...
        if (passwordInput.isEmpty()) {
            textInputPassword.setError("Проверьте пароль");
            return false;
        } else if (passwordInput.length() < 7) {
            textInputPassword.setError("Не мении 6  символов");
            return false;
        }   else {
    ...
    private boolean validateConfirmPassword() {
        ...
        if (!passwordInput.equals(confirmPasswordInput)) {
            textInputPassword.setError("Пароль должен быть одинаковый");
            return false;
        } else {
            textInputPassword.setError("");
            return true;
...
//                                updateUI(null);
            ...
            auth.createUserWithEmailAndPassword(textInputEmail.getEditText().getText().toString().trim(),
                    textInputConfirmPassword.getEditText().getText().toString().trim())
                    .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (task.isSuccessful()) {
...
//                                updateUI(null);
    ...
    public void toggleLoginSignUp(View view) {
        ...
        if (isLoginModeActive){
            ...
            loginSignIpButton.setText("Зарегистрируйтесь");
            toggleLoginSignUpTextView.setText("или Войти");
            textInputConfirmPassword.setVisibility(View.VISIBLE);
        } else {
            isLoginModeActive = true;
            loginSignIpButton.setText("Войти");
